...
(function(){
  ...
  async function openSocial(type){const o=document.getElementById('socialOverlay'),p=document.getElementById('socialPanel');o.classList.remove('hidden');
    if(type==='profile'){
      const x=prof();p.innerHTML='<h2>PROFILE</h2><label>PLAYER NAME</label><input id="pname" maxlength="16" value="'+String(x.name).replace(/"/g,'&quot;')+'" readonly><label>AVATAR TAG</label><input id="pavatar" maxlength="3" value="'+String(x.avatar).replace(/"/g,'&quot;')+'"><div class="socialActions"><button class="primary" id="saveP">SAVE</button><button id="closeP">CLOSE</button></div>';document.getElementById('saveP').onclick=()=>{x.avatar=(document.getElementById('pavatar').value||'FN').trim().slice(0,3).toUpperCase()||'FN';saveProf(x);renderProfile();o.classList.add('hidden')};document.getElementById('closeP').onclick=()=>o.classList.add('hidden');
    }else if(type==='friends'){
      let friends=[],requests={incoming:[],outgoing:[]};try{friends=(await fnApi('/friends/list')).friends||[];requests=await fnApi('/friends/requests')}catch(e){p.innerHTML='<h2>FRIENDS</h2><p>FAILED: '+e.message+'</p><button id="closeF">CLOSE</button>';document.getElementById('closeF').onclick=()=>o.classList.add('hidden');return}
      p.innerHTML='<h2>FRIENDS</h2><label>ADD FRIEND NAME</label><div class="socialActions"><input id="friendName" maxlength="16" placeholder="PLAYER"><button class="primary" id="addFriend">ADD</button></div><div id="friendStatus"></div><h3>REQUESTS</h3><div id="friendRequests">'+(requests.incoming||[]).map(r=>'<div class="friend-row"><span>'+r.name+'</span><button data-rid="'+r.id+'" data-act="accept">ACCEPT</button><button data-rid="'+r.id+'" data-act="reject">REJECT</button></div>').join('')+(requests.incoming?.length?'':'<div class="fn-admin-note">NO PENDING REQUESTS</div>')+'</div><h3>FRIEND LIST</h3><div id="friendList">'+(friends.length?friends.map(f=>'<div class="friend-row"><span>'+f.name+'</span><small>PLAYER</small></div>').join(''):'<div class="fn-admin-note">NO FRIENDS YET</div>')+'</div><div class="socialActions"><button id="closeF">CLOSE</button></div>';
      document.getElementById('addFriend').onclick=async()=>{const name=document.getElementById('friendName').value.trim();try{await fnApi('/friends/request',{method:'POST',body:JSON.stringify({targetName:name})});document.getElementById('friendStatus').textContent='REQUEST SENT';setTimeout(()=>openSocial('friends'),250)}catch(e){document.getElementById('friendStatus').textContent='FAILED: '+e.message}};
      p.querySelectorAll('[data-rid]').forEach(btn=>btn.onclick=async()=>{try{await fnApi('/friends/respond',{method:'POST',body:JSON.stringify({requestId:Number(btn.dataset.rid),action:btn.dataset.act})});openSocial('friends')}catch(e){alert(e.message)}});document.getElementById('closeF').onclick=()=>o.classList.add('hidden');
    }else{
      let room=null;try{room=(await fnApi('/rooms/current')).room}catch(e){}
      if(!room){p.innerHTML='<h2>PRIVATE ROOM</h2><div class="socialActions"><button class="primary" id="createPokerRoom">CREATE POKER ROOM</button><button id="joinRoomBtn">JOIN ROOM</button><button id="closeR">CLOSE</button></div><div id="roomStatus"></div>';document.getElementById('createPokerRoom').onclick=async()=>{try{const d=await fnApi('/rooms/create',{method:'POST',body:JSON.stringify({gameType:'poker'})});openSocial('room');setTimeout(()=>{const st=document.getElementById('roomStatus');if(st)st.innerHTML='<b>ROOM CODE '+d.room.code+'</b>'},0)}catch(e){document.getElementById('roomStatus').textContent='FAILED: '+e.message}};document.getElementById('joinRoomBtn').onclick=()=>{p.innerHTML='<h2>JOIN ROOM</h2><input id="joinCode" inputmode="numeric" maxlength="4" placeholder="4-DIGIT CODE"><div class="socialActions"><button class="primary" id="joinNow">JOIN</button><button id="backRoom">BACK</button></div>';document.getElementById('joinNow').onclick=async()=>{try{const d=await fnApi('/rooms/join',{method:'POST',body:JSON.stringify({code:document.getElementById('joinCode').value})});renderRoom(d.room)}catch(e){alert(e.message)}};document.getElementById('backRoom').onclick=()=>openSocial('room')};document.getElementById('closeR').onclick=()=>o.classList.add('hidden');
      }else renderRoom(room);
    }
  }
  async function renderRoom(room){const o=document.getElementById('socialOverlay'),p=document.getElementById('socialPanel');p.innerHTML='<h2>POKER ROOM</h2><div class="roomCode"><small>ROOM CODE</small><strong>'+room.code+'</strong></div><div><small>HOST: '+room.host_player_id+'</small></div><div id="roomPlayers">'+room.members.map(m=>'<div class="friend-row"><span>'+m.name+'</span><small>'+ (m.ready?'READY':'WAITING') +'</small></div>').join('')+'</div><div class="socialActions"><button class="primary" id="readyRoom">READY</button><button id="leaveRoom">LEAVE</button></div><div class="fn-admin-note">Poker online room base is ready. Game sync will attach in Phase 2.</div>';document.getElementById('readyRoom').onclick=async()=>{try{await fnApi('/rooms/ready',{method:'POST',body:JSON.stringify({ready:true})});const d=await fnApi('/rooms/current');renderRoom(d.room)}catch(e){alert(e.message)}};document.getElementById('leaveRoom').onclick=async()=>{try{await fnApi('/rooms/leave',{method:'POST'});openSocial('room')}catch(e){alert(e.message)}}
  }
