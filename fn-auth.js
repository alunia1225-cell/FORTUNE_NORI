...
(function(){
  ...
  function ensureUI(){
    ...
    const n=$('fnLoginName'),p=$('fnLoginPassword'),b=$('fnLoginButton');
  ...
  async function login(rawName,rawPass){
    const name=String(rawName||'').trim(),pass=String(rawPass||'');
  ...
  ensureUI();showDebug(false);gateTap();
